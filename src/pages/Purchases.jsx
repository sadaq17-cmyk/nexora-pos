...
import { Plus, X, Save, Trash2, CheckCircle2, RotateCcw } from "lucide-react";
...
export default function Purchases() {
  ...
  const handleCreate = async (e) => {
    ...
    const result = await api.purchases.create({ supplier_id: Number(supplierId), invoice_no: invoiceNo || null, items, status: "Pending" });
    if (result.success) {
      showToast(`Purchase order ${result.po_number} created`);
      setModalOpen(false); setSupplierId(""); setInvoiceNo(""); setLines([{ product_id: "", qty: 1, cost: "" }]);
      await load();
    } else showToast(result.error || "Failed to create purchase order");
  };

  const markReceived = async (po) => {
    const result = await api.purchases.receive(po.id);
    if (result.success) { showToast(`${po.po_number} marked Received — stock and supplier balance updated`); await load(); }
    else showToast(result.error || "Could not receive this order");
  };
  ...
  const submitReturn = async (e) => {
    e.preventDefault();
    if (!returnLine.product_id || !returnLine.qty) { showToast("Choose a product and quantity"); return; }
    ...
    if (result.success) {
      showToast("Return recorded — stock and supplier balance updated");
      setReturnFor(null);
      await load();
    } else showToast(result.error || "Could not record return");
  };
    ...
    <div className="animate-fadein">
      <div className="flex items-start justify-between mb-5">
        <div>
          ...
          <p className="text-sm text-[#6B7690] mt-0.5">Purchase orders, receiving, and returns.</p>
        </div>
        ...
        {loading ? (
          ...
          <table className="w-full">
            <thead><tr className="bg-[#F3F6FB]">
              {["PO Number", "Supplier", "Invoice", "Date", "Items", "Total", "Status", "Actions"].map((h) => (
            ...
            <tbody>
              {purchases.map((po) => {
                ...
                return (
                  <tr key={po.id} className="border-t border-[#E4E9F2] hover:bg-[#F8FAFD]">
                    ...
                    <td className="px-4 py-3 text-sm font-mono text-[#6B7690]">{po.invoice_no || "—"}</td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        {po.status !== "Received" && can("purchases", "edit") && (
                        ...
                        {po.status === "Received" && can("purchases", "edit") && (
                          <button onClick={() => openReturn(po)} className="flex items-center gap-1 text-xs font-medium text-[#DC2626] hover:underline">
              ...
              {purchases.length === 0 && <tr><td colSpan={8} className="text-center py-10 text-sm text-[#6B7690]">No purchase orders yet.</td></tr>}
            </tbody>
          </table>
        )}
      </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg p-5 max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleCreate} className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-xs font-medium text-[#1B2439] mb-1 block">Supplier invoice #</label>
                  <input value={invoiceNo} onChange={(e) => setInvoiceNo(e.target.value)} placeholder="Optional" className="w-full border border-[#E4E9F2] rounded-lg px-3 py-2 text-sm" />
                </div>
              </div>
      ...
      {returnFor && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4" onClick={() => setReturnFor(null)}>
          <div className="bg-white rounded-2xl w-full max-w-sm p-5" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-[#1B2439]">Return Items — {returnFor.po_number}</h3>
              <button onClick={() => setReturnFor(null)} className="text-[#6B7690]"><X size={18} /></button>
            </div>
            <form onSubmit={submitReturn} className="space-y-3">
              <select required value={returnLine.product_id} onChange={(e) => setReturnLine({ ...returnLine, product_id: e.target.value })} className="w-full border border-[#E4E9F2] rounded-lg px-3 py-2 text-sm">
                <option value="">Product on this order…</option>
                {purchaseItems.map((it) => <option key={it.id} value={it.product_id}>{it.product_name}</option>)}
              </select>
              <div className="grid grid-cols-2 gap-3">
                <input required type="number" min={1} placeholder="Qty to return" value={returnLine.qty} onChange={(e) => setReturnLine({ ...returnLine, qty: e.target.value })} className="w-full border border-[#E4E9F2] rounded-lg px-3 py-2 text-sm" />
                <input type="number" step="0.01" placeholder="Unit cost" value={returnLine.cost} onChange={(e) => setReturnLine({ ...returnLine, cost: e.target.value })} className="w-full border border-[#E4E9F2] rounded-lg px-3 py-2 text-sm" />
              </div>
              <input placeholder="Reason (e.g. damaged, wrong item)" value={returnLine.reason} onChange={(e) => setReturnLine({ ...returnLine, reason: e.target.value })} className="w-full border border-[#E4E9F2] rounded-lg px-3 py-2 text-sm" />
              <button type="submit" className="w-full flex items-center justify-center gap-1.5 py-2.5 rounded-lg text-white text-sm font-semibold bg-[#DC2626] hover:brightness-110">
