...
export default function SubscriptionRenew() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const planList = await api.plans?.listPublic?.()
        || await api.subscription?.getPlans?.()
        || await api.owner?.getPlatformConsole?.().then((r) => r.plans).catch(() => null);
      if (Array.isArray(planList) && planList.length) {
        setPlans(planList.filter((p) => PAID_PLAN_CODES.includes(p.code) && p.active !== false));
      } else {
        setPlans(CANONICAL_PLANS.filter((p) => PAID_PLAN_CODES.includes(p.code)));
      }
    })();
  }, []);
