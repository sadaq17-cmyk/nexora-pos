...
export function localDateRange(startDate, endDate) {
...
function saleAdjustments(sale) {
  const returned = Math.max(
    ...
    Array.isArray(sale.returns) ? sale.returns.reduce((sum, row) => sum + number(row.amount ?? row.refund), 0) : 0
  );
  const refundCandidate = Math.max(
    number(sale.refund),
    number(sale.refund_amount),
    number(sale.refunded_amount),
    Array.isArray(sale.refunds) ? sale.refunds.reduce((sum, row) => sum + number(row.amount), 0) : 0
  );
  return {
    returns: returned,
...
function aggregate(db, filters, startDate, endDate) {
  ...
  const users = new Map((db.users || []).map((row) => [String(row.id), row]));
  const productTotals = new Map();
  const categoryTotals = new Map();
  const cashierTotals = new Map();
  const daily = new Map(dateSequence(range.start, range.end).map((key) => [key, { date: key, sales: 0, profit: 0, expenses: 0, transactions: 0 }]));
  const hourly = Array.from({ length: 24 }, (_, hour) => ({ hour: `${String(hour).padStart(2, "0")}:00`, sales: 0, transactions: 0 }));

  const result = {
    revenue: 0, transactions: 0, cashSales: 0, discounts: 0, vat: 0,
    returns: 0, refunds: 0, cogs: 0, expenses: 0,
    salesRows: [], productTotals, categoryTotals, cashierTotals, daily, hourly,
  };

  const hasItemFilter = !!(filters.product_id || filters.category_id);
  for (const sale of db.sales || []) {
    if (!inRange(sale.created_at, range)) continue;
    if (filters.branch_id && !idEquals(sale.branch_id ?? db.settings?.default_branch_id, filters.branch_id)) continue;
    if (filters.cashier_id && !idEquals(sale.user_id, filters.cashier_id)) continue;

    const allItems = Array.isArray(sale.items) ? sale.items : [];
    const matchedItems = allItems.filter((item) => {
      const product = products.get(String(item.product_id)) || {};
      const categoryId = item.category_id ?? product.category_id;
      return (!filters.product_id || idEquals(item.product_id, filters.product_id))
    ...
    const discount = number(sale.discount ?? sale.discount_amount ?? sale.discountAmt) * ratio;
    const vat = number(sale.vat ?? sale.vat_amount) || (
      sale.vat_enabled && number(sale.vat_rate) > 0
        ? Math.max(0, explicitSubtotal - number(sale.discount)) * number(sale.vat_rate) / 100
        : 0
    );
    ...
    const cashier = users.get(String(sale.user_id));
    const cashierName = sale.user_name || sale.cashier_name || cashier?.name || (sale.user_id ? "Unknown" : "System");
    ...
    const cashierRow = cashierTotals.get(String(sale.user_id ?? "system")) || { id: sale.user_id ?? "system", name: cashierName, revenue: 0, transactions: 0 };
    ...
    for (const item of sourceItems) {
      ...
      const itemRevenue = Math.max(0, number(item.price) * number(item.qty) - discount * itemRatio - adjustments.total * ratio * itemRatio);
      const productRow = productTotals.get(String(item.product_id)) || { id: item.product_id, name: item.name || product.name || "Unknown product", units: 0, revenue: 0 };
      productRow.units += number(item.qty);
      productRow.revenue += itemRevenue;
      productTotals.set(String(item.product_id), productRow);
      const categoryRow = categoryTotals.get(String(categoryId)) || { id: categoryId, name: item.category || product.category || categories.get(String(categoryId))?.name || "Uncategorized", units: 0, revenue: 0 };
  ...
  for (const expense of db.expenses || []) {
    ...
    const branchId = expense.branch_id ?? db.settings?.default_branch_id;
    if (filters.branch_id && !idEquals(branchId, filters.branch_id)) continue;
...
export function buildReportAnalytics(db, filters = {}) {
  ...
  return {
    ...
    charts: {
      ...
      profit: dailyTrend.map((row) => ({ date: row.date, profit: row.profit, revenue: row.sales, expenses: row.expenses })),
    ...
    options: {
      ...
      cashiers: (db.users || []).filter((user) => user.active !== 0).map(({ id, name }) => ({ id, name })),
