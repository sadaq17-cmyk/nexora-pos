...
**Sender address:** `noreply@httpsnexorapos.com`
**Domain:** `httpsnexorapos.com`
...
2. Enter `httpsnexorapos.com` and choose the region closest to your users.
3. Resend will show you a list of DNS records to add — typically:
   ...
   - Sometimes a **Return-Path/MX** record scoped to a subdomain (e.g. `send.httpsnexorapos.com`) used for bounce tracking.
...
4. Add each record in **Cloudflare** (same place your Email Routing is already configured):
   - Log into Cloudflare → select the `httpsnexorapos.com` zone → **DNS** → **Records**.
...
6. Once the domain shows **Verified** in Resend, outbound sending from `@httpsnexorapos.com` addresses is ready.
...
- **After** both are done: real verification and password-reset emails will be delivered to real inboxes from `noreply@httpsnexorapos.com`.
