 ...
 * Covers: receive, inventory, supplier balance, payments, audit, stock movements.
 */
...
// Ensure a mock session with owner rights if mockApi requires it
if (typeof mockApi.__setTestUser === "function") {
  mockApi.__setTestUser({ id: "owner-1", role: "owner", name: "Owner", company_id: 1 });
}
...
assert.ok(productAfter, "product missing after receive");
assert.equal(
...
assert.ok(supplierAfter, "supplier missing after receive");
const expectedBalance = balanceBefore + Number(create.total || qty * cost);
assert.ok(
...
assert.ok(
  ["Received", "PartiallyReceived"].includes(String(po.status)),
  `unexpected status ${po.status}`
);

// Partial payment if supported
if (typeof mockApi.purchases.addPayment === "function") {
  const payAmt = Math.min(10, Number(po.total || expectedBalance) || 10);
...
// Audit / stock movements — mock may expose via audit.getAll or inventory helpers
